...
class GCN(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        self.conv2 = GCNConv(hidden_dim, output_dim)  # Output dimension should be 1 for binary classification
        self.sigmoid = nn.Sigmoid()

    def forward(self, x, edge_index):
        x = self.conv1(x.to(torch.float32), edge_index, edge_weight=edge_weight.to(torch.float32))
        x = torch.relu(x)
