...
def process_adjacency_matrix(file_path, features,threshold=500) :
    ...
    edge_weight = torch.where(adj_matrix == 0, torch.zeros_like(adj_matrix), 1.0 / adj_matrix)
...
# Example usage:
file_path = "weights1.csv"
edge_weight, edge_index = process_adjacency_matrix(file_path, features)
...
adj_matrix = pd.read_csv("weights1.csv", header=None).to_numpy()
adj_matrix[adj_matrix > 500] = 0
adj_matrix = torch.tensor(adj_matrix)
...
class SimpleGCN(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, dropout_rate=0.5, weight_decay=0.001):
        ...
        self.conv2 = GCNConv(hidden_dim, hidden_dim)
        self.conv3 = GCNConv(hidden_dim, output_dim)
        #self.attention = nn.MultiheadAttention(hidden_dim, 2)  # Reduced to 2 attention heads
        self.norm1 = nn.LayerNorm(hidden_dim)
        self.norm2 = nn.LayerNorm(hidden_dim)
...
#train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, dr)
train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, drop_last=True)

val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=True)

# Create the ComplexGCN model
model = SimpleGCN(input_dim=76, hidden_dim=64, output_dim=19).to(device)
