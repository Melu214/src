...
def plot_sub_data(sub_data, cid):
    # Features to plot
    features = ['Temp', 'Sal', 'DO_mgl', 'errors']
...
def analyze_sub_data(sub_data):
    ...
    for feature in ['Temp', 'Sal', 'DO_mgl' ]:
...
folder_path = 'C:/Users/mi0025/Documents/Research/anomalydetector/'

# Define the date range
start_date = '2007-01-01'
end_date = '2023-09-30'

filename = 'weeksbayData.csv'
...
mainData = mainData[['datetime', 'Temp', 'Sal','DO_mgl']]
...
mainData = mainData[(mainData['datetime'] >= start_date) & (mainData['datetime'] <= end_date)]

mainData.columns = ['datetime', 'Temp', 'Sal', 'DO_mgl']
...
merged_data.columns = ['Temp', 'Sal', 'DO']
...
fig, axes = plt.subplots(nrows=3, ncols=1, figsize=(14, 10), dpi=80)
...
axes[0].plot(train['DO'], label='DO', color='green', linewidth=1)
...
fig_fft, axes_fft = plt.subplots(nrows=3, ncols=1, figsize=(14, 10), dpi=80)
...
axes_fft[0].plot(np.abs(train_fft[:, 0]), label='DO', color='green', linewidth=1)
axes_fft[0].set_title('Dissolved Oxygen (Frequency Domain)')
...
model.save("autoencoder_modelEsturyv6.h5")
...
anomalies = (normalized_errors > 1 * normalized_errors.std()).astype(int)
...
result_df[label] = normalize_and_add_count(df.copy(), label,min_series_length=30, max_gap_between_series=12)['hurricaneCount']
...
for value in unique_values:
    ...
    if not subset.empty:
        ...
        start_window = start_time - pd.Timedelta(days=10)
        end_window = end_time + pd.Timedelta(days=10)
...
# Assuming 'datetime' is a datetime column and other features are numeric
# Make sure to sort the data by datetime if it's not already sorted
sub_data = sub_data.sort_values(by='datetime')

# Extract the relevant columns
datetime = sub_data['datetime']
features = ['Temp', 'Sal', 'DO_mgl']

# Calculate the time difference in seconds
time_diff_seconds = (datetime.iloc[-1] - datetime.iloc[0]).total_seconds()

# Perform Fourier transform for each feature
plt.figure(figsize=(12, 8))

for feature in features:
    data = sub_data[feature]
    ...
    n = len(data)
    freq = np.fft.fftfreq(n, d=time_diff_seconds / (n - 1))
    
    # Plot the amplitude spectrum for each feature
    plt.plot(freq, np.abs(fft_result), label=feature)

plt.title('Frequency Domain Analysis - Amplitude Spectrum')
...
for feature in features:
    ...
    # Identify peaks in the amplitude spectrum
    peaks, _ = find_peaks(np.abs(fft_result))
    
    # Plot the amplitude spectrum for each feature
    plt.plot(freq, np.abs(fft_result), label=feature)
    
    # Identify the frequency corresponding to the peak
    peak_frequency = np.abs(freq[peaks[0]])

    # Find the timestamps corresponding to peak and normal values
    peak_timestamp = datetime.iloc[data.idxmax()]
    normal_timestamp = datetime.iloc[data.idxmin()]
    
    # Calculate the durations
    duration_to_peak = (peak_timestamp - datetime.iloc[0]).total_seconds()
    duration_to_normal = (normal_timestamp - peak_timestamp).total_seconds()

    print(f'{feature}:')
    print(f'Duration to Peak: {duration_to_peak} seconds')
    print(f'Duration from Peak to Normal: {duration_to_normal} seconds\n')
